...
| 事件名称       | 类型 | 参数描述                     |
| -------------- | :--: | ---------------------------- |
| onRecognize    | func | 识别结果回调                 |
| onStartRecord  | func | 开始录音回调                 |
| onStopRecord   | func | 结束录音回调                 |
| onUpdateVolume | func | 录音音量(单位为分贝)实时回调 |
| onError        | func | 通用的错误事件               |
...
| 事件名称                  | 类型 | 参数描述                                                                  |
| ------------------------- | :--: | ------------------------------------------------------------------------- |
| onSliceSuccessRecognize   | func | 每个语音包分片识别结果                                                    |
| onSegmentSuccessRecognize | func | 语音流的识别结果,一次识别中可以包括多句话，这里持续返回的每句话的识别结果 |
| onSuccessRecognize        | func | 一次识别任务最终的结果                                                    |
| onErrorRecognize          | func | 一次识别任务失败回调                                                      |
| onStartRecord             | func | 开始录音回调                                                              |
| onStopRecord              | func | 结束录音回调                                                              |
| onUpdateVolume            | func | 录音音量(单位为分贝)实时回调                                              |
| onSilentDetectTimeOut     | func | 静音检测超时回调, 仅支持Android                                           |
| onSaveAudioDataAsFile     | func | 录音停止后回调一次，再次开始录音会清空上一次保存的文件。                  |
| onError                   | func | 通用的错误事件                                                            |
