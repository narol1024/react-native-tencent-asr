...
- (NSArray<NSString *> *)supportedEvents {
  return @[
    @"FlashFileRecognizerModule.onError",
  ];
  ...
  -(void)initializeRecognizer {
    ...
    if ([_token isEqual:@""]) {
      _recognizer =
          [[QCloudFlashFileRecognizer alloc] initWithAppId:_appId
                                                  secretId:_secretId
  ...
  RCT_EXPORT_METHOD(configure : (NSDictionary *)configParams) {
    ...
    requestParams[@"wordInfo"] =
        @([configParams[@"wordInfo"] integerValue] ?: 1);
  ...
  // 上传文件成功回调
  -(void)FlashFileRecognizer
      : (QCloudFlashFileRecognizer *_Nullable)recognizer status
      : (nullable NSInteger *)status text : (nullable NSString *)text resultData
