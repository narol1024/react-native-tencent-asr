...
static NSString *_moduleName = @"FlashFileRecognizerModule";
...
RCT_EXPORT_METHOD(recognize
  ...
  NSLog(@"%@, 调用recognize方法, 调用参数: %@", _moduleName, configParams);
...
- (void)FlashFileRecognizer:(QCloudFlashFileRecognizer *_Nullable)recognizer
  ...
  if (status == 0) {
    NSLog(@"%@, 识别成功", _moduleName);
    _recognizerResolve(resultData);
  } else {
    NSLog(@"%@, 识别失败: %@", _moduleName, text);
    _recognizerReject([NSString stringWithFormat:@"%ld", (long)status], text,
                      nil);
  }
}
